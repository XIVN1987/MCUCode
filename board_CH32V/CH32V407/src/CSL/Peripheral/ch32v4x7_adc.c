...
void ADC_DeInit(ADC_TypeDef *ADCx)
{
    ...
    {
        RCC_PB2PeriphResetCmd(RCC_PB2Periph_ADC2, ENABLE);
        RCC_PB2PeriphResetCmd(RCC_PB2Periph_ADC2, DISABLE);
    }
}
...
FlagStatus ADC_GetFlagStatus(ADC_TypeDef *ADCx, uint32_t ADC_FLAG)
{
    ...
    if((ADCx->STATR & ADC_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
s32 TempSensor_Volt_To_Temper(s32 Value)
{
    ...
    Refer_Volt = (s32)((*(u32 *)TS_BASE) & 0x0000FFFF);
    Refer_Temper = (s32)(((*(u32 *)TS_BASE) >> 16) & 0x0000FFFF);
 ...
 *            ADC_Channel_16 - ADC Channel16 selected.
 *            ADC_Channel_17 - ADC Channel17 selected.
 *          SMP_Mode - configer SMPx mode.
 *            ADC_SMP_CFG_MODE0 - SMPx mode 0.
 *            ADC_SMP_CFG_MODE1 - SMPx mode 1.
 *
 * @return  none
 */
