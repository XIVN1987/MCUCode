...
{
    RCC_HBPeriphResetCmd(RCC_HBPeriph_I3C, ENABLE);
    RCC_HBPeriphResetCmd(RCC_HBPeriph_I3C, DISABLE);
}
...
void I3C_Ctrl_Config(I3C_CtrlConfTypeDef* I3C_CtrlConf)
{
    ...
    cfgr_value = ((uint32_t)I3C_CtrlConf->HotJoinAllowed_EN << 7) | ((uint32_t)I3C_CtrlConf->PullUp << 6);
...
void I3C_SetConfigFifo(I3C_FifoConfTypeDef* I3C_FifoConf)
{
    ...
    if (I3C->CFGR & I3C_CFGR_CRINIT)
    {
        value |= (I3C_FifoConf->ControlFifo);
        mask |= (I3C_CFGR_TMODE);
    }
 ...
 *            I3C_ERROR_COVR - C FIFO Under-Run error 
