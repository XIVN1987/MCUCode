...
#define ValidAddrEnd               (FLASH_BASE + 0xF8000)
 ...
 *          OB_RAM_MODE - FLASH and Ram configuration mode
 *            OB_RAM_CODE_MOD1 - CODE 576 KB RAM 136 KB
 *            OB_RAM_CODE_MOD0 - CODE 512 KB RAM 200 KB
 *            
 * @return  FLASH Status - The returned value can be: FLASH_BUSY, FLASH_ERROR_PG,
 *        FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
 */
FLASH_Status FLASH_UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_STOP, uint16_t OB_STDBY,uint16_t OB_RAM_MODE)
{
    FLASH_Status status = FLASH_COMPLETE;
    uint32_t     Addr = 0x1FFFF800;
    __IO uint8_t i;
    ...
    if(status == FLASH_COMPLETE)
    {
        ...
        /* Write optionbytes */
        pbuf[1] = OB_IWDG | (uint16_t)(OB_STOP | (uint16_t)(OB_STDBY | (uint16_t)(OB_RAM_MODE | ((uint16_t)temp))));
 ...
 *          The FLASH User Option Bytes values - RST_STOP(Bit1)
 *          The FLASH User Option Bytes values - RST_STDBY(Bit2)
 *          The FLASH and RAM mode configuration = RAM_CODE_MOD(BIt6)
 */
 ...
 *            FLASH_Access_SYSTEM - SystemCoreClock
 *            FLASH_Access_SYSTEM_HALF - SystemCoreClock/2
 *            FLASH_Access_SYSTEM_FOURTH - SystemCoreClock/4
 *
 * @return  none
 */
void FLASH_Access_Clock_Cfg(uint32_t FLASH_Access_CLK)
{
    uint32_t tmp = FLASH->CTLR;
    tmp &= ~(3 << 25);
    tmp |= FLASH_Access_CLK;
    FLASH->CTLR = tmp;
}
 ...
 *
 * @param   StartAddr - Erases Flash start address(StartAddr%4096 == 0).
 *          Cnt - Erases count.
 ...
 *
 * @param   StartAddr - Erases Flash start address(StartAddr%4096 == 0).
 *          Length - Erases Flash start Length(Length%4096 == 0).
 *
