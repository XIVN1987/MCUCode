...
/* I2S slave sync */
#define I2S_ENABLE_SYNC       ((uint32_t)(SPI_HSCR_I2CWSSYNC))
 ...
 * @fn      I2S_SlaveSyncWS
 *
 * @brief   Enables WS signal-based synchronization for SPI peripheral 
 *          when operating in I2S slave mode.
 *
 * @param   SPIx - where x can be 2 or 3 to select the SPI peripheral.
 *          NewState - new state of the Sync state.
 *
 * @return  none
 */
void I2S_SlaveSyncWS(SPI_TypeDef *SPIx, FunctionalState NewState)
{
    if(NewState != DISABLE)
    {
        SPIx->HSCR |= I2S_ENABLE_SYNC;
    }
    else
    {
        SPIx->HSCR &= ~I2S_ENABLE_SYNC;
